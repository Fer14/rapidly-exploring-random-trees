...
def main():
    ...
    while running:
        ...
        if is_is_obstacle(random_node, screen):
            continue
        ...
        if (
            ...
            if not goal_reached(new_node, screen):
    ...
    text_ = font_.render("      Path found!", True, black)
    textRect_ = text_.get_rect()
    textRect_.center = (240, 490)
    screen.blit(text_, textRect_)
    # restat_button = Button(black, 200, 510, 100, 30, "Restart")
    # restat_button.create(screen)
    pygame.display.update()
